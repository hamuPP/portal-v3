...
<template>
    <div class="selection-box-new" :class="{label: haveLabel()}">
        ...
        <div ref="inputBox" class="ipt-box">
            <input class="ipt"
                   ...
                   :selection-obj="nowId"
                   ref="selectIpt"
                   :placeholder="objProps.placeholder"
                   @click="iptClickEvt"
                   @keyup="keyupEvt"
                   v-model="objProps.filterValue"/>
            <i class="icon" :class="[{'active': showBox}]" :selection-obj="nowId" @click="iptClickEvt"></i>
        </div>
        <div class="box" ref="box" v-show="showBox" :style="[{'top': pos.t + 'px'}, {'width': boxW + 'px'}]">
            <div class="block">
...
<script>
    export default{
        ...
        methods: {
            ...
            _selectListItem() {
                let that = this;
                let listArray = that.objProps.list;

                // 循环找到list中对应的项目，并选中
                for (let i = 0, len = listArray.length; i < len; i++) {
                    let item = listArray[i];
                    if (item.name === that.objProps.filterValue) {
                        item._select_show = true;
                        return;
                    }
                }
        ...
        created() {
            ...
            this.objProps.placeholder = this.objProps.placeholder || '请输入关键字';
            /* 修改选中默认日期的方式 by:ty 2018年03月23日11:03:53 以前的无法使用用户默认初始值 */
            if (this.objProps.filterValue) {
                // 选中list列表中对应的项目
                this._selectListItem();
               // this.$set(this.objProps, "filterValue", this.objProps.list[0].checked  ? this.objProps.list[0].name : '');
            } else { // 如果没有，则选中第一项
                this.$set(this.objProps, 'filterValue', this.objProps.list[0].checked ? this.objProps.list[0].name : '');
            }
        }
